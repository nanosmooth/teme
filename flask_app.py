...
db_path = os.path.join(BASE_DIR, "textthree.db")
...
def writeToLoggedIn( str ):
  ...
  conn.execute("INSERT INTO MESSAGES (NAME,TEXT,ROU,POSTTIME) \
      VALUES (?,?,?,?)",(session['username'],htmlstr,'U',post_time));
  conn.commit()
  conn.close()
...
def welcome():
 ...
 if not session.get('logged_in'):
    ...
    sql="SELECT * FROM MESSAGES WHERE ROU!='U' or NAME='%s' LIMIT 50 OFFSET %d-50" % (session.get('username'),tot_rows)
    ...
    for row in cursor:
        ...
        if row[0] == session.get('username'):
            ...
            msg='%s%s%s</div>%s%s%s</div></div>' % (mainlt,sublt,row[0],msgtxt,sublt,post_time)
        msgarr.append(msg)
...
def receive():
   ...
   for row in cursor:
       msgtxt = emoApple(row[1])
       msg='%s%s%s</div>%s%s%s</div></div>' % (mainlt,sublt,row[0],msgtxt,sublt,row[3])
       msgarr.append(msg)
...
def load_prev_msgs():
    ...
    sql="SELECT * FROM MESSAGES WHERE ROU!='U' or NAME='%s' LIMIT 50 OFFSET %d-%d" % (session.get('username'),tot_rows,session.get('offset_val'))
